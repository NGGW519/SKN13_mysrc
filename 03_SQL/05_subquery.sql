...
-- 직원_ID 120번 직원을 먼저 조회한 후 같은 업무 직원을 또 조회한다. 2번의 조회하는 구조
...
order by 3 desc;
-- where salary > avg(salary); 이렇게 쓰면 안된다. 
...
order by 3 desc;
-- concat으로 숫자들이 문자열이 되어버려 정렬하면 숫자가 큰 순서대로 정렬하는게 아니라, 1이 먼저 나오도록 정렬된다.
-- 따라서 서브쿼리를 아래처럼 이용
select dept_id,
       dept_name,
       concat('$', round(평균급여, 2)) as "평균급여" -- 서브쿼리를 먼저 실행하면서 '평균급여'가 나왔으므로 메인에서 그냥 별칭을 써줘도된다.
...
-- null에 대해서 적지 않으면, null은 모르는 값이므로 비교연산자를 하면 무조건 false라서 null도 제외하고 나온다.
...
-- 서브쿼리 where절의 비교 컬럼(loc)이 unique가 아닐 경우 => 결과행이 여러개일 수 있다. 다중행 서브쿼리 연산!
...
select * from emp
where salary > (select salary from emp where emp_id in (101,102,103));
...
-- 중복된 데이터 조회 (emp에서 여러번 나오는 이름을 조회, 컬럼이름 포함)
select emp_name, cnt
from (
...
-- 조건을 만족하는 행이 여러개라도 한행만 있으면 더이상 검색하지 않는다. 있는지 없는지만 알려주지, 몇 번 있는지는 안알려준다.
...
-- 서브쿼리에거 메인쿼리의 테이블을 사용하도록 정의하면 상관쿼리
