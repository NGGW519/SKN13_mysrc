...
*************************************************************************************************************** */
use hr;
-- 들어가기 앞서, 함수로 값을 변경하면, 컬럼의 제목에는 함수가 그대로 현출되기 때문에 별칭으로 컬럼 제목을 보기 좋게 해주는게 좋다.
...
######################################
-- 숫자에 ,같은 단위 구분자 찍고 앞에 $ 붙이기
-- 괄호 안의 숫자는 소수점 몇째자리까지 보여줄지 정하는 것
...
select emp_id,
	   emp_name,
       salary,
       ceil(salary * 1.15) as "SAL_RAISE"  -- 내림을 하고싶으면 floor()
from emp;
...
select emp_id,
       ...
       salary as "SAL",
       ceil(salary * 1.15) as "SAL_RAISE",  -- 내림을 하고싶으면 floor()
       ceil(salary * 1.15) - salary as "SAL_DIFFER"  -- SAL_RAISE - salary 처럼 별칭으로 계산할 수 없다. 
from emp;



######################################
--  EMP 테이블에서 커미션이 있는 직원들의 직원_ID(emp_id), 이름(emp_name), 커미션비율(comm_pct), 커미션비율(comm_pct)을 8% 인상한 결과, 백의 자리에서 반올림한 급여(salary)를 조회.
-- (단 커미션을 8% 인상한 결과는 소숫점 이하 2자리에서 반올림하고 별칭은 comm_raise로 지정, null 나오는 애들은 지워주기)
select emp_id,
	   emp_name,
       comm_pct,
...
######################################
-- 실행시점의 일/시를 조회 함수
-- DB에서 자료를 가져오는게 아니라서 from을 생략한채로 쓴다.
select now(); -- datetime type
select curdate(); -- date type
select curtime(); -- time type

-- 아래와 같이 insert할때도 함수를 넣어서 할 수 있다.
-- insert into member (id, ......., join_date) values(?,?, ......,curdate())
...
select curdate(), -- date type
       ...
       month(curdate()),
       day(curdate()),
       dayofweek(curdate());  -- 요일 조회 숫자로 알려줌

######################################
-- 시간 타입에서 시 분 초 조회
-- 사실 시간 타입이 아니라도, datetime 타입처럼 시간에 관한 자료가 있는거면 시간 조회가능
select curtime(),
	   hour(curtime()),
       minute(curtime()),
       second(curtime());
...
select emp_name,
       ...
       date_format(hire_date, '%m월 %Y년 %d일'),
       date_format(hire_date, '%Yyear %mmonth %dday'),
       date_format(hire_date, '%dday %Yyear %mmonth')
from emp
where emp_id = 200;
...
from emp
order by timestampdiff(month, hire_date, now()) desc;


######################################
-- 270개월 이상 근무한 직원들 조회.
select * from emp
where timestampdiff(month, hire_date, now()) >= 270;
...
select emp_id,
	   emp_name,
       job,
       ifnull(dept_name, '부서 미배치') as dept_name  -- '부서미배치'는 한글이라 맨 뒤로가있다.
from emp
order by dept_name;
...
######################################
-- comm_pct가 없으면 "커미션 없음"을, 있으면 "커미션 있음"을 출력하도록 한다.
select emp_id,
	   emp_name,
       salary,
       salary * comm_pct as "comm",
       if(comm_pct is null, "커미션 없음", "커미션 있음") as "커미션 유무"
from emp;
...
######################################
-- 숫자를 날짜로 변환
select convert('20250404', date),  -- 문자열 type
	   convert(20250404, date),  -- 숫자 type
	   convert('123456', time);


######################################
-- 숫자를 문자열로 변환
select convert('2334.56', char);

/* *************************************
CASE 문
if하나로 하기엔 조건이 많은경우, else에 if넣고, else에 if 넣으면서 할 순 있지만,
--->>> case를 쓴다
case문 동등비교.  -> 하나의 컬럼만을 비교값과 비교할때
case 컬럼 when 비교값 then 출력값   -> 비교값 자리는 값이 나와야지 연산자 같은거 못나온다.
              [when 비교값 then 출력값]
              [else 출력값]
              end
              
case문 조건문.  -> 여러개 컬럼을 사용할땐 이걸로 조건문에 일일이 다 기입한다.
...
select case dayofweek(curdate()) when 1 then '일요일'   -- 쉼표없음!
								 when 2 then '월요일'
                                 when 3 then '화요일'
                                 when 4 then '수요일'
                                 when 5 then '목요일'
...
-- case when salary < 5000 then salary * 0.3
     -- when salary >= 10000 then salary * 0.1
     -- when salary between 5000 and 10000 then salary * 0.2