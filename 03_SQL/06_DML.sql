-- DML은 DB나 테이블을 변경하는 것이다. 테이블 기준으로한다.
-- 여러 테이블을 한꺼번에 하지 않는다.
...
update emp
set comm_pct = null -- 변경에서는 is null을 사용 안함.(where절에서는 is null로 비교)
where dept_id = 100;
...
delete from dept where dept_id = 10;
   -- 삭제하면서 null로 바뀐다. 따라서 null의 개수가 늘어난걸 확인가능
