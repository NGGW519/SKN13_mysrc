...
- 인수(argument)는 컬럼.
  ...
  - min(): 최소값
  - max(): 최대값
  - stddev(): 표준편차 = 편차의 평균 -> 편차 = 평균으로부터의 거리
  - variance(): 분산 = 표준편차**2
  - count(): 개수
        - 인수: 
...
- 문자타입/일시타입: max(), min(), count()에만 사용가능
	- 문자열 컬럼의 max(): 사전식 배열에서 가장 마지막 문자열, min()은 첫번째 문자열.
		- (min)특수문자 > 숫자 > 알파벳 대문자 > 알파벳 소문자 > 한글(max)
        - 주의 10000>600 이지만, '10000'<'600' 임을 주의! 문자열에서의 숫자는 사전찾는 것처럼!
...
select max(hire_date) as "최근 입사일",
	   min(hire_date) as "오래된 입사일"
from emp;


#########################
-- EMP 테이블의 부서(dept_name) 의 개수를 조회
select count(dept_name) from emp;  -- 부서가 있는 직원의 수나 다름없다.

select distinct dept_name from emp; -- 부서의 고유값들만 조회

select count(distinct dept_name) from emp; -- 부서 몇개? null포함x

select distinct ifnull(dept_name, 'a') from emp; -- null을 포함한 개수를 세기 위해서 null에 a를 넣어 고유값 조회

select count(distinct ifnull(dept_name, 'a')) from emp; -- 부서 몇개? null포함o
...
from emp;
-- 주의 위 문제에서 몇글자인지 & 그 이름은 무엇인지 조회를 원할때?
-- select emp_name, max(char_length(emp_name)) 이렇게 적으면 안된다. emp_name은 여러 값을 갖는 일반컬럼인데, max집계한건 값 1개짜리라서 붙일 수 없다.
...
#########################
-- 입사년도와(hire_date) 업무(job)가 같은 직원들의 평균 급여(salary)을 조회. 연도 기준으로
select year(hire_date),
       job,
	   avg(salary)
from emp
...
- 구문  : 내가 집계할 그룹에 대한 조건을 넣어준다. 조건에 맞는 TRUE들만 모아서 집계
    having 제약조건  
...
from emp
group by dept_name
having count(*) >= 10; -- *는 나눠진 그룹의 전체를 의미함
